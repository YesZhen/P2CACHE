...
PREINSTALL_PKGS=openssh-server,curl,tar,gcc,make,g++,libc6-dev,time,strace,sudo,less,psmisc
...
echo "p2cache" | sudo tee $DIR/etc/hostname
...
sudo mkdir -p $DIR/mnt/data
sudo mkdir -p $DIR/mnt/sdb
sudo mkdir -p $DIR/root/share
sudo cp -r /usr/local/lib/filebench $DIR/usr/local/lib/
# set up directory sharing
echo 'mount -t 9p -o trans=virtio,version=9p2000.L hostshare /root/share' | sudo tee -a $DIR/root/.profile
echo 'export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/root/tmpdir/lib' | sudo tee -a $DIR/root/.profile
echo 'export PATH=$PATH:/root/tmpdir/bin' | sudo tee -a $DIR/root/.profile
# disable aslr
...
sudo rm $DIR/etc/security/limits.conf
echo 'root               soft    nofile          1000000000' | sudo tee -a $DIR/etc/security/limits.conf
echo 'root               hard    nofile          1000000000' | sudo tee -a $DIR/etc/security/limits.conf
echo '# End of file' | sudo tee -a $DIR/etc/security/limits.conf
echo 'DefaultLimitNOFILE=1000000000' | sudo tee -a $DIR/etc/systemd/user.conf
echo 'DefaultLimitNOFILE=1000000000' | sudo tee -a $DIR/etc/systemd/system.conf
echo 'session required pam_limits.so' | sudo tee -a $DIR/etc/pam.d/common-session
...
sudo chroot $DIR /bin/bash -c "apt-get update -y --allow-unauthenticated; apt-get install -y --allow-unauthenticated ipmctl ndctl libtool m4 automake bison byacc flex tree python2 python3 xfsprogs fio jq libsnappy-dev libgflags-dev net-tools"
...
# sudo chroot $DIR /bin/bash -c "apt-get update -y --allow-unauthenticated; apt-get install -y -f --allow-unauthenticated openjdk-8-jdk maven"
# sudo chroot $DIR /bin/bash -c "wget -qO - https://www.mongodb.org/static/pgp/server-6.0.asc | sudo apt-key add -"
# sudo chroot $DIR /bin/bash -c "echo 'deb [ arch=amd64,arm64 ] https://repo.mongodb.org/apt/ubuntu focal/mongodb-org/6.0 multiverse' | sudo tee /etc/apt/sources.list.d/mongodb-org-6.0.list"
# sudo chroot $DIR /bin/bash -c "apt-get update -y --allow-unauthenticated; apt-get install -y --allow-unauthenticated mongodb-org"

# sudo chroot $DIR /bin/bash -c "wget https://dl.min.io/server/minio/release/linux-amd64/archive/minio_20220107015323.0.0_amd64.deb -O minio.deb"
# sudo chroot $DIR /bin/bash -c "wget https://dl.min.io/client/mc/release/linux-amd64/archive/mcli_20220105235251.0.0_amd64.deb -O mc.deb"
# sudo chroot $DIR /bin/bash -c "dpkg -i minio.deb"
# sudo chroot $DIR /bin/bash -c "dpkg -i mc.deb"
