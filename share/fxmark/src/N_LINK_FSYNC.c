...
{
    struct fx_opt *fx_opt = fx_opt_worker(worker);
    sprintf(test_file, "%s/%d/n_link_fsync-%" PRIu64 ".dat",
...
static int pre_work(struct worker *worker)
{
    ...
    for (;worker->private[0] < 1000000; ++worker->private[0]) {
...
static int main_work(struct worker *worker)
{
    ...
    set_test_root(worker, old_path);

    if ((dir_fd = open(old_path, O_DIRECTORY, 0)) == -1)
        goto err_out;

    for (iter = 0; iter < worker->private[0] && !bench->stop; ++iter) {
        set_test_file(worker,   iter, old_path);
		set_test_file(worker, iter+1000000, new_path);
...
struct bench_operations n_link_fsync_ops = {
    .pre_work  = pre_work,
    .main_work = main_work,
};
