...
static int pre_work(struct worker *worker)
{
	...
	/* create a test file */ 
	snprintf(file, PATH_MAX, "%s/n_ar_4k.dat", test_root);
    ...
    // for(loff_t pos = 0; pos + len <= FILE_SIZE; pos += 4096) {
    //     if(pwrite(fd, page, len, pos) != len)
    //         goto err_out;
    // }
...
static int main_work(struct worker *worker)
{
	...
	// //   	pos = 0;
	//   if (pread(fd, page, len, pos) != len)
	// 	goto err_out;
	// }
	for (iter = 0; !bench->stop; ++iter, pos += len) {
	  if (pos + len > FILE_SIZE)
	  	pos = 0;
	  if (pread(fd, page, len, pos) != len)
		goto err_out;
...
struct bench_operations n_ar_4k_ops = {
	.pre_work  = pre_work, 
	.main_work = main_work,
};
